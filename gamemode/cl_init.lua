...
function GM:CalcView(Ply, Position, Angles, FOV)
	...
	Ply.ZoomDist = Ply.ZoomDist or 100
	Ply.BestLap = Ply.BestLap or 0
...
function GM:HUDPaint()
	...
	Ply.Respawning = Ply.Respawning or 0
	...
	local NameW = math.max(60, surface.GetTextSize(self.Stats.BestLapName))
	...
	draw.SimpleTextOutlined(Ply.Laps or 0 .. "/" .. self.NumLaps, "ScoreboardText", 205, 245, Color(255, 255, 255, 255), 0, 0, 1, Color(0, 0, 0, 255))


	-- Health panel --
	local Melon = Ply:GetNWEntity("Melon")
	if IsValid(Melon) then
		local HP, MaxHP = Melon:GetNWInt("HP") or 1, Melon:GetNWInt("MaxHP") or 1
	...
	for k,v in pairs(player.GetAll()) do
		local Melon = v:GetNWEntity("Melon")
	...
	if !self.HidePowerupNames then
		for k,v in pairs(ents.FindByClass("sent_powerup")) do
			if (IsValid(v)) then
				...
				Trace.filter = {Ply, Ply:GetNWEntity("Melon")}
				Trace.mask = MASK_SOLID
				Trace = util.TraceLine(Trace)

				if !Trace.Hit then
					-- Get our powerup type
					local PowerupType = v:GetNWString("PowerupType", nil)
...
local function Melonracer_Countdown()
	GAMEMODE.Countdown = CurTime() + 3
	GAMEMODE.GameStarted = false
	GAMEMODE:ResetVars()
end
net.Receive("Melonracer_Countdown", Melonracer_Countdown)

-- Usermessage 'Melonracer RespawnMelon' --
local function Melonracer_RespawnMelon()
	local Ply = LocalPlayer()
	if !IsValid(Ply) then return end
	Ply.Respawning = CurTime() + net.ReadFloat()
end
net.Receive("Melonracer_RespawnMelon", Melonracer_RespawnMelon)

-- Usermessage 'Melonracer RightWay' --
local function Melonracer_Rightway()
	local Ply = LocalPlayer()
	if !IsValid(Ply) then return end

	Ply.Way = net.ReadUInt(2)
	Ply.WayTimer = CurTime() + 3

	if (Ply.Way == 1) then
		-- It's a checkpoint, get the current checkpoint number
		Ply.Checkpoint = net.ReadUInt(8)
	elseif (Ply.Way == 2) then
		-- It's a lap, we sent the current lap too, get it
		Ply.Laps = net.ReadUInt(8)
	end
end
net.Receive("Melonracer_RightWay", Melonracer_Rightway)

-- Usermessage 'Melonracer SetLapStart' --
local function Melonracer_SetLapStart()
	local Ply = LocalPlayer() -- This is needed at some places because global doesn't exst yet
	Ply.LapStart = net.ReadFloat()
end
net.Receive("Melonracer_SetLapStart", Melonracer_SetLapStart)

-- Usermessage 'Melonracer SetBestLap' --
local function Melonracer_SetBestLap()
	local Ply = net.ReadEntity()
	Ply.BestLap = net.ReadFloat()
end
net.Receive("Melonracer_SetBestLap", Melonracer_SetBestLap)

-- Usermessage 'Melonracer SetLastLap' --
local function Melonracer_SetLastLap()
	local Ply = LocalPlayer()
	if !IsValid(Ply) then return end
	Ply.LastLap = net.ReadFloat()
end
net.Receive("Melonracer_SetLastLap", Melonracer_SetLastLap)

-- Usermessage 'Melonracer SetServerBestLap' --
local function Melonracer_SetServerBestLap()
	GAMEMODE.Stats.BestLap = net.ReadFloat()
	GAMEMODE.Stats.BestLapPrint = string.ToMinutesSecondsMilliseconds(GAMEMODE.Stats.BestLap)
	GAMEMODE.Stats.BestLapName = net.ReadString()
end
net.Receive("Melonracer_SetServerBestLap", Melonracer_SetServerBestLap)

-- Usermessage 'Melonracer SetLeader' --
local function Melonracer_SetLeader()
	-- local Pos = net.ReadUInt(2) -- unused
	local Name = net.ReadString()
	local Name2 = net.ReadString()
	local Name3 = net.ReadString()
...
net.Receive("Melonracer_SetLeader", Melonracer_SetLeader)

-- Usermessage 'Melonracer ResetVars' --
local function Melonracer_ResetVars()
	GAMEMODE:ResetVars()
end
net.Receive("Melonracer_ResetVars", Melonracer_ResetVars)
