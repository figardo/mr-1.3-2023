...
util.AddNetworkString("Melonracer_SetLapStart")
util.AddNetworkString("Melonracer_RespawnMelon")
util.AddNetworkString("Melonracer_RightWay")
util.AddNetworkString("Melonracer_Countdown")
util.AddNetworkString("Melonracer_ResetVars")
util.AddNetworkString("Melonracer_SetLeader")
util.AddNetworkString("Melonracer_SetServerBestLap")
util.AddNetworkString("Melonracer_SetLastLap")
util.AddNetworkString("Melonracer_SetBestLap")

util.AddNetworkString("sent_melon_RecieveHP")
util.AddNetworkString("sent_melon_BombTimer")
...
function GM:NewMatch(Wait)
	...
	timer.Simple(Wait, function()
		for k,v in pairs(player.GetAll()) do
			...
			net.Start("Melonracer_Countdown")
				-- Nothing really
			net.Send(v)
...
function GM:ResetVars(Ply)
	if (IsValid(Ply)) then
		...
		net.Start("Melonracer_ResetVars") net.Send(Ply)
	end
end
...
function GM:UpdateTopThree()
	...
	net.Start("Melonracer_SetLeader")
		-- net.WriteUInt(0, 2)
		net.WriteString(self.Stats.FirstPlace)
		net.WriteString(self.Stats.SecondPlace)
		net.WriteString(self.Stats.ThirdPlace)
	net.Send(Filter)
end
	...
	if self.GameStarted and (FrameTime() != 0) then
		for k,v in pairs(player.GetAll()) do
			...
			if (v:HasMelon()) then
				...
				if v:KeyPressed(IN_ATTACK) and v.CheckpointPos then
